			...
			<div class="tag-list-wrapper" @click.self="onClickTags">
				...
				<svws-ui-icon
					...
					><i-ri-arrow-up-s-line
						v-if="focused"
						class="svws-ui-pointer-events-none" /><i-ri-arrow-down-s-line
						v-else
		...
		<div v-show="input" class="input">
			...
			>
				...
				<svws-ui-icon class="svws-ui--dropdown--icon"
					><i-ri-arrow-up-s-line
						v-if="focused" /><i-ri-arrow-down-s-line v-else
				/></svws-ui-icon>
			</label>
		</div>
	...
	export default defineComponent({
		...
		methods: {
			...
			select() {
				if (!this.list) return;
				this.select_item(
					this.filtered_list[this.selected_item_index - 1]
				);
			},
			...
			onArrowDown() {
				...
				else if (this.selected_item_index === list)
...
<style scoped>
	...
	.tags {
		...
		@apply svws-ui-h-auto svws-ui-overflow-hidden;
	}

	.dropdown-icon {
		@apply svws-ui-absolute svws-ui-right-0 svws-ui-flex-shrink-0 svws-ui-bg-white svws-ui-py-1 svws-ui-px-1;
	}
	...
	.svws-ui--multiselect--items-wrapper {
		@apply svws-ui-absolute svws-ui-z-20 svws-ui-max-h-64 svws-ui-w-full;
		@apply svws-ui-divide-y svws-ui-divide-light;
		@apply svws-ui-rounded svws-ui-border svws-ui-border-t-0 svws-ui-border-gray;
		@apply svws-ui--mt-2 svws-ui-pt-2;
