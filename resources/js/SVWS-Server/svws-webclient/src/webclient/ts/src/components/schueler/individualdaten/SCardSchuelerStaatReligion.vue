	...
	<svws-ui-content-card title="Staatsangehörigkeit und Konfession">
		<div class="input-wrapper">
			<svws-ui-multi-select v-model="inputStaatsangehoerigkeit" title="1. Staatsangehörigkeit"
				:items="inputKatalogStaatsangehoerigkeit" :item-text="(i: Nationalitaeten) => i.daten.staatsangehoerigkeit"
...
const inputKatalogReligionen: ComputedRef<List<ReligionKatalogEintrag>> =
	computed(() => {
...
const inputReligionID: WritableComputedRef<
	ReligionKatalogEintrag | undefined
> = computed({
	get(): ReligionKatalogEintrag | undefined {
		const id = daten.value.religionID;
		let o;
		for (const r of inputKatalogReligionen.value) { if (r.id === id) { o = r; break } }
		return o;
	},
	set(val) {
...
const inputReligionabmeldung: WritableComputedRef<string | undefined> =
	computed({
...
const inputReligionanmeldung: WritableComputedRef<string | undefined> =
	computed({
		get(): string | undefined {
...
const inputDruckeKonfessionAufZeugnisse: WritableComputedRef<boolean> =
	computed({
		get(): boolean {
