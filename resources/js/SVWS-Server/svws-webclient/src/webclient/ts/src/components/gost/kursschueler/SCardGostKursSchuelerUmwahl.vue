		...
		<div class="test flex-none">
			<div class="sticky">
				...
				<div class="rounded-lg shadow">
					<svws-ui-checkbox v-model="filter_kollision" class="px-5"
						>Nur Kollisionen</svws-ui-checkbox
					>
					<svws-ui-text-input
						v-model="filter_name"
						type="search"
				...
				<div class="overflow-hidden rounded-lg shadow">
					...
					>
						<i-ri-delete-bin-2-line
							class="m-2 text-4xl"
							:class="{ 'text-red-700': is_dragging }"
						/>
					</div>
				</div>
			</div>
...
<script setup lang="ts">
	...
	const schienen: ComputedRef<
		Array<GostBlockungsergebnisSchiene> | undefined
	> = computed(() => {
		return app.dataKursblockungsergebnis.daten?.schienen.toArray(
			new Array<GostBlockungsergebnisSchiene>()
		);
	});
	...
	> = computed(() => {
		const schienen = app.dataKursblockungsergebnis.daten?.schienen.toArray(
			new Array<GostBlockungsergebnisSchiene>()
		);
		if (!schienen) return new Map();
		const map = new Map();
		for (const k of kurse.value) {
			map.set(
				...
				schienen.map(s =>
					s.kurse
						.toArray(new Array<GostBlockungsergebnisKurs>())
						.find(kk => k.id === kk.id)
				)
			);
		}
	...
	const fachbelegungen: ComputedRef<Array<AbiturFachbelegung> | undefined> =
		computed(() => {
			const belegungen: Array<AbiturFachbelegung> | undefined =
				app.dataSchuelerLaufbahndaten.daten?.fachbelegungen.toArray(
					new Array<AbiturFachbelegung>()
				);
			if (!belegungen) return undefined;
			belegungen.sort((a, b) => {
	...
	const filter_name: WritableComputedRef<string | undefined> = computed({
