		...
		<div v-if="app?.stammdaten.daten" class="flex w-full flex-col px-4">
			...
			<svws-ui-tab-bar>
				<template #tabs>
					...
					<svws-ui-tab-button v-if="hatAbiturjahrgang">Laufbahnplanung</svws-ui-tab-button>
				</template>
				<template #panels>
					<svws-ui-tab-panel>
					...
					<svws-ui-tab-panel v-if="hatAbiturjahrgang">
...
import { Main, mainInjectKey } from "~/apps/Main";
import { Klassen } from "~/apps/klassen/Klassen";
import { Schueler } from "~/apps/schueler/Schueler";

const main: Main | undefined = inject(mainInjectKey);
const app: Schueler | undefined = main?.apps.schueler;
const appKlassen: Klassen | undefined = main?.apps.klassen;
...
const inputNachname: ComputedRef<string | false> = computed(() => {
	if (app?.auswahl.ausgewaehlt) {
		return app.auswahl.ausgewaehlt.nachname.toString();
	}
	return false;
});
...
const inputVorname: ComputedRef<string | false> = computed(() => {
	if (app?.auswahl.ausgewaehlt) {
		return app?.auswahl.ausgewaehlt.vorname.toString();
	}
	return false;
});
