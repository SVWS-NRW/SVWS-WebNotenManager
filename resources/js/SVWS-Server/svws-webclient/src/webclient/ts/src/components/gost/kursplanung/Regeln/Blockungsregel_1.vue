...
const regel_entfernen = async (regel: GostBlockungRegel) => {
...
<template>
	<div>
		...
		<div v-for="r in regeln" :key="r.id" class="flex justify-between">
			<div>{{GostKursart.fromID(r.parameter.get(0).valueOf()).beschreibung}}, von Schiene {{r.parameter.get(1)}} bis {{r.parameter.get(2)}} gesperrt</div>
